...
const SCREENS = ['login', 'setup', 'list', 'editor', 'orders'] as const;
type Screen = typeof SCREENS[number];

export class AdminApp {
  private auth: AuthService;
  private brand: BrandSettingsService;
  private editor: ProductEditor;
  ...
  async loadList(): Promise<void> {
    ...
    if (unowned && unowned.length > 0) {
      document.getElementById('claim-count')!.textContent = String(unowned.length);
    ...
    el.innerHTML = this._products.map(p => this._productRow(p)).join('');
    this._initDragDrop(el);
  }

  private _productRow(p: Product): string {
  ...
  private _initDragDrop(el: HTMLElement): void {
    ...
    el.addEventListener('drop', async e => {
      e.preventDefault();
      const targetRow = (e.target as HTMLElement).closest<HTMLElement>('[data-id]');
      ...
      this._initDragDrop(el);
      await supabase.from('products').upsert(reordered.map((p, i) => ({ id: p.id, sort_order: i })));
    });
  }

  private _bindGlobals(): void {
    window.show              = (n: string) => this.show(n as Screen);
    ...
    window.updateOrderStatus = async (id: string, status: string) => { const ok = await this.orders.updateStatus(id, status); if (ok) this.orders.load(); };
    window.openEditor        = async (id: string | null) => { await this.editor.open(id); this.show('editor'); };
    window.saveProduct       = async () => { const ok = await this.editor.save(); if (ok) this.loadList(); };
    window.removeExistingImage = (i: number) => this.editor.removeExistingImage(i);
    window.toggleHidden      = async (id: string, current: boolean) => {
    ...
    window.deleteProduct     = async (id: string, name: string) => {
  ...
  async init(): Promise<void> {
    ...
    if (session) {
      (document.getElementById('header-shop-link') as HTMLAnchorElement).href = `${location.origin}/?brand=${this.auth.currentUser!.id}`;
