...
export class ProductService {
  ...
  async fetchNextPage(brandId: string): Promise<void> {
    ...
    grid.insertAdjacentHTML('beforeend', (data as Product[]).map(p => this._card(p)).join(''));
    this.page++;
    if (data.length < PAGE_SIZE) this.done = true;
  }

  async getById(id: string, brandId: string): Promise<Product | null> {
  ...
  initInfiniteScroll(brandId: string): void {
  ...
  private _card(p: Product): string {
